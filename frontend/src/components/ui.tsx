    ...
    <button
      ...
      className={cn(
        ...
        variant === 'primary'
          ...
          : 'border border-ink-600 text-slate-700 hover:border-ink-500 hover:text-slate-900',
        className,
      )}
    >
      {children}
  ...
  return (
    <span
      className={cn(
        ...
        verdict ? VERDICT_STYLES[verdict] : 'border-ink-500 bg-ink-700 text-slate-700',
        className,
      )}
    >
      {children}
    ...
    <div>
      {label && (
        <div className="mb-1 flex justify-between text-xs text-slate-400">
          <span>{label}</span>
          <span className="font-mono text-slate-700">{percent(value)}</span>
        </div>
      )}
...
export function StatTile({
  ...
  return (
    <div className="rounded-lg border border-ink-600 bg-ink-700/50 px-4 py-3">
      ...
      <div className="mt-1 font-mono text-xl text-slate-900">{value}</div>
