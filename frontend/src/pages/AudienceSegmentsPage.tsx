          ...
          {roots.map((root) => (
            <div key={root.id} className="space-y-3">
              <div className="flex items-baseline gap-3">
                <h2 className="text-sm font-semibold text-slate-700">{root.name}</h2>
              ...
              <ul className="space-y-2 text-sm">
                {data.adjacency.map((edge) => (
                  <li key={`${edge.fromSegmentId}-${edge.toSegmentId}`} className="flex items-center gap-3">
                    <span className="text-slate-700">{nameOf(data.segments, edge.fromSegmentId)}</span>
                    <span className="text-slate-600">→</span>
                    <span className="text-slate-700">{nameOf(data.segments, edge.toSegmentId)}</span>
