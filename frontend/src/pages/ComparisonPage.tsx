...
export default function ComparisonPage() {
  ...
  return (
    <>
      <PageHeader
        title="Original vs Variants"
        description={result.hypothesis}
        action={<Badge>{result.modificationType}</Badge>}
      />
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <Card className="border-ink-500">
              <div className="mb-2 flex items-center justify-between">
                <h3 className="text-sm font-semibold text-slate-100">Original</h3>
                  ...
                  <tbody>
                    {segmentIds(result.comparison).map((segmentId) => (
                      <tr key={segmentId} className="border-b border-ink-700/60">
                        <td className="py-2 pr-4 text-slate-300">{nameOf(segmentId)}</td>
                        {result.variants.map((variant) => {
  ...
  return (
    <Card className={isWinner ? 'border-signal-strong/40' : undefined}>
      <div className="mb-2 flex items-start justify-between gap-2">
        <h3 className="text-sm font-semibold text-slate-100">{variant.label}</h3>
        <span className="font-mono text-lg text-slate-200">
      ...
      <blockquote className="mt-3 border-l-2 border-accent-dim pl-3 text-sm italic leading-relaxed text-slate-300">
